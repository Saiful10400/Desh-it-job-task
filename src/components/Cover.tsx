import modelImage from "../assets/cover/model.webp"
import CoverButton from "./sub-components/CoverButton";
import CenterAlign from "./utils/CenterAlign";
const Cover = () => {
    return (
        <div className="bg-[#333333]">
            <CenterAlign>
                <div className="flex justify-between items-center lg:flex-row flex-col">
                    {/* cover text and buttons. */}
                    <div className="flex flex-col gap-[30px] lg:min-w-[600px] lg:max-w-1/2">
                        <h3 className="text-[20px] text-[#ffbd39] poppins-extrabold">Hello !</h3>
                        <p className="text-[26px] sm:text-[30px] leading-[36px] poppins-extrabold">I'm Professional Web Designer &<br /> <span className="text-[#ffba3a]">Wordpress Developer</span></p>
                        <p className="text-lg roboto-regular">It is a long established fact that a reader will be distracted by the readable <br /> content of a page when looking at its layout.</p>
                        <div className="flex gap-6">
                            <CoverButton bg="#7c68ee" text="Hire Me" />
                            <CoverButton bg="#ffbd3a" text="Download Cv" />
                        </div>
                    </div>

                    {/* cover model image. */}
                    <div className="lg:max-w-[50%] max-h-full lg:h-[750px]">
                        <img className="w-full h-full object-contain" src={modelImage} alt="model image." />
                    </div>
                </div>
            </CenterAlign>
        </div>
    );
};

export default Cover;