...
const PortfolioImageFullScreen = ({ isActive, fullscreenFn }: { fullscreenFn: React.Dispatch<React.SetStateAction<boolean>>, isActive: boolean }) => {
    ...
    return (
        <div ref={containerRef} className={`${isActive ? "block" : "hidden"} fixed top-0 left-0 w-screen h-screen bg-[#000000bb] z-50 flex justify-between px-[32px] items-center`}>
            <button onClick={() => imageChangeHandle("down")} className="cursor-pointer"><ArrowLeftIcon /></button>
            <div className="flex flex-col justify-center items-end">
                <button onClick={() => fullscreenFn(false)} className="text-2xl text-gray-400 hover:text-white transition-all duration-300 cursor-pointer">x</button>
